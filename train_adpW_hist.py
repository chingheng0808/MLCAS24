...
from model_adpW_hist import model_Regression_add
import clip
...
PREFIX = "Latest"
ARCH = "L1L2_ADP_stat_lstm_lrc_64nf_4rb_64h_stepLR_3ai"
CHANNEL = 12
IMGSIZE = (20, 10)
...
def coarse_train(batch_size=32, lr=0.0005, epoch=120):
    ...
    train_dataset = HIPSDataset_fast(
        root="/ssd8/chingheng/IPPS/data_latest", single=False, ishist=True
    )
    ...
    for i in range(epoch):
        ...
        for data in train_loader:
            x, t, gt, ai, hist = data["hsi"], data["prompt"], data["gt"], data["add_info"], data["hist"]
            # print(x.shape)
            optimizer.zero_grad()
